...
open class InboxRecyclerView @JvmOverloads constructor(
  ...
  fun expandItem(
    ...
    if (isLaidOut.not()) {
      post { expandItem(itemAdapterPosition, immediate) }
      return
    }
...
//    }
    ...
    expandedItem = ExpandedItem(itemViewPosition, itemAdapterPosition, itemRect)
    if (immediate) {
...
//        )
  ...
  data class ExpandedItem(
    ...
    companion object {
      internal val EMPTY =
        ExpandedItem(adapterPosition = -1, viewIndex = -1, locationOnScreen = Rect(0, 0, 0, 0))
    }
  }
}
