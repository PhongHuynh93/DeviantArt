 ...
 */
internal class TopicDetailDataSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Art> {
        return try {
            ...
            val map = mapOf("offset" to nextPageNumber.toString(), "limit" to 24.coerceAtMost(params.loadSize)
                .toString(), "topic" to topicName)
            val response = restApi.getTopic(map)
